...
The gnmi-exporter automates gNMI streaming telemetry collection from network devices managed by [network-operator](https://github.com/ironcore-dev/network-operator). It watches `DeviceMonitor` custom resources and `Device` objects to continuously reconcile the desired monitoring state.

<img :src="architectureImage" alt="gNMI Exporter Architecture" />
