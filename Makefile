...
docker-build: FORCE
ifdef GITHUB_TOKEN
	@printf "\e[1;36m>> $(CONTAINER_TOOL) build --build-arg=GITHUB_TOKEN --tag=$(IMG) .\e[0m\n"
	@$(CONTAINER_TOOL) build --build-arg=BININFO_BUILD_DATE=$(BININFO_BUILD_DATE) --build-arg=BININFO_COMMIT_HASH=$(BININFO_COMMIT_HASH) --build-arg=BININFO_VERSION=$(BININFO_VERSION) --build-arg=GITHUB_TOKEN=$(GITHUB_TOKEN) --tag=$(IMG) .
else
	@printf "\e[1;36m>> $(CONTAINER_TOOL) build --ssh=default --tag=$(IMG) .\e[0m\n"
	@$(CONTAINER_TOOL) build --build-arg=BININFO_BUILD_DATE=$(BININFO_BUILD_DATE) --build-arg=BININFO_COMMIT_HASH=$(BININFO_COMMIT_HASH) --build-arg=BININFO_VERSION=$(BININFO_VERSION) --ssh=default --tag=$(IMG) .
endif
...
build-all: build/monitoring-operator

build/monitoring-operator: FORCE generate
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=monitoring-operator -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/monitoring-operator ./cmd
...
install: FORCE build/monitoring-operator
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/monitoring-operator "$(DESTDIR)$(PREFIX)/bin/monitoring-operator"
...
build/cover.out: FORCE generate install-setup-envtest | build
	...
	KUBEBUILDER_ASSETS=$$(setup-envtest use 1.33 -p path) go run github.com/onsi/ginkgo/v2/ginkgo run --randomize-all -output-dir=build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=monitoring-operator -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -covermode=count -coverpkg=$(subst $(space),$(comma),$(GO_COVERPKGS)) $(GO_TESTFLAGS) $(GO_TESTPKGS)
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/cobaltcore-dev/monitoring-operator\e[0m\n"
	@goimports -w -local github.com/cobaltcore-dev/monitoring-operator $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/monitoring-operator\e[0m    Build monitoring-operator.\n"
